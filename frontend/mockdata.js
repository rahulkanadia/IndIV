export const mockData = {
    spot: 26150,
    strikes: Array.from({length: 21}, (_, i) => 26000 + (i * 50)),
    
    // For Skew Chart
    skew: {
        call: [13.5, 13.2, 13.0, 12.8, 12.6, 12.5, 12.4, 12.4, 12.5, 12.6, 12.8, 13.0, 13.3, 13.6, 13.9, 14.3, 14.7, 15.2, 15.8, 16.4, 17.0],
        put: [15.5, 15.0, 14.6, 14.2, 13.9, 13.6, 13.4, 13.2, 13.0, 12.9, 12.8, 12.8, 12.9, 13.0, 13.2, 13.5, 13.9, 14.4, 15.0, 15.7, 16.5],
        spread: [2.0, 1.8, 1.6, 1.4, 1.3, 1.1, 1.0, 0.8, 0.5, 0.3, 0.0, -0.2, -0.4, -0.6, -0.7, -0.8, -0.8, -0.8, -0.7, -0.5]
    },
    skewMo: {
        call: [14.5, 14.4, 14.3, 14.2, 14.1, 14.0, 14.0, 14.0, 14.1, 14.2, 14.3, 14.5, 14.7, 15.0, 15.3, 15.7, 16.1, 16.6, 17.1, 17.7, 18.3],
        put: [16.0, 15.8, 15.6, 15.4, 15.2, 15.0, 14.8, 14.7, 14.6, 14.5, 14.5, 14.6, 14.7, 14.9, 15.2, 15.5, 15.9, 16.4, 17.0, 17.7]
    },
    
    // For Term Structure Chart
    term: {
        weekly: [12.8, 14.5, 15.2, 15.8, 16.2],
        monthly: [14.5, 15.0, 15.5, 16.0, 16.5],
        expiries: ['26 Dec', '02 Jan', '09 Jan', '16 Jan', '30 Jan']
    },
    
    // For Surface Charts

    // 1. Existing Monthly Data (Default)
    surfMoney: { 
        z: [
            // Dec (Smile shape: High edges, Low ATM)
            [14.5, 14.0, 13.6, 13.2, 12.8, 12.5, 12.8, 13.2, 13.6, 14.0, 14.5], 
            // Jan
            [15.5, 15.0, 14.6, 14.3, 14.1, 14.0, 14.1, 14.3, 14.6, 15.0, 15.5], 
            // Feb
            [16.5, 16.0, 15.7, 15.5, 15.3, 15.2, 15.3, 15.5, 15.7, 16.0, 16.5]
        ], 
        x: ['-10', '-8', '-6', '-4', '-2', 'ATM', '+2', '+4', '+6', '+8', '+10'], 
        y: ['Dec', 'Jan', 'Feb'] 
    },
    surfDelta: { 
        z: [[14,13,14],[15,15,16],[16,16,17]], 
        x: ['20D', '50D', '80D'], 
        y: ['Dec', 'Jan', 'Feb'] 
    }, 

    // 2. NEW Weekly Data (For the Toggle)

    // UPDATED: Full list of Weekly Expiries (9 Weeks)
    surfMoneyWk: { 
        z: [
            // 26 Dec
            [14.0, 13.5, 13.0, 12.6, 12.3, 12.0, 12.4, 12.7, 13.1, 13.6, 14.1],
            // 02 Jan
            [14.2, 13.8, 13.4, 13.0, 12.7, 12.5, 12.8, 13.1, 13.5, 13.9, 14.3],
            // 09 Jan
            [14.5, 14.1, 13.7, 13.4, 13.1, 13.0, 13.2, 13.5, 13.8, 14.2, 14.6],
            // 16 Jan
            [14.7, 14.3, 13.9, 13.6, 13.3, 13.2, 13.4, 13.7, 14.0, 14.4, 14.8],
            // 23 Jan
            [15.0, 14.6, 14.2, 13.9, 13.6, 13.5, 13.7, 14.0, 14.3, 14.7, 15.1],
            // 30 Jan
            [15.2, 14.8, 14.4, 14.1, 13.8, 13.7, 13.9, 14.2, 14.5, 14.9, 15.3],
            // 06 Feb
            [15.4, 15.0, 14.6, 14.3, 14.0, 13.9, 14.1, 14.4, 14.7, 15.1, 15.5],
            // 13 Feb
            [15.7, 15.3, 14.9, 14.6, 14.3, 14.2, 14.4, 14.7, 15.0, 15.4, 15.8],
            // 20 Feb
            [16.0, 15.6, 15.2, 14.9, 14.6, 14.5, 14.7, 15.0, 15.3, 15.7, 16.1]
        ], 
        x: ['-10', '-8', '-6', '-4', '-2', 'ATM', '+2', '+4', '+6', '+8', '+10'], 
        y: ['26 Dec', '02 Jan', '09 Jan', '16 Jan', '23 Jan', '30 Jan', '06 Feb', '13 Feb', '20 Feb'] 
    },
    surfDeltaWk: { 
        z: [
            [13.0, 12.2, 13.5], // 26 Dec
            [13.5, 13.0, 14.0], // 02 Jan
            [14.2, 13.8, 14.8], // 09 Jan
            [14.5, 14.0, 15.0], // 16 Jan
            [14.8, 14.5, 15.5], // 23 Jan
            [15.5, 15.0, 16.2], // 30 Jan (Monthly)
            [15.8, 15.2, 16.5], // 06 Feb
            [16.0, 15.5, 16.8], // 13 Feb
            [16.5, 16.0, 17.0]  // 20 Feb
        ], 
        x: ['20D', '50D', '80D'], 
        y: ['26 Dec', '02 Jan', '09 Jan', '16 Jan', '23 Jan', '30 Jan', '06 Feb', '13 Feb', '20 Feb'] 
    },
    
    // For Intraday Chart
    intraday: {
        time: ["09:15", "10:15", "11:15", "12:15", "13:15"],
        wk: [12.2, 12.4, 12.1, 12.5, 12.8],
        mo: [14.1, 14.0, 14.2, 14.3, 14.4],
        wkRv: [10.5, 10.5, 10.5, 10.5, 10.5],
        moRv: [11.0, 11.0, 11.0, 11.0, 11.0]
    },

    // For Grids (Price Boxes)
    gridWeekly: {
        date: "26 DEC 2025",
        atmCall: { val: 145.20, chg: "+12 (9%)", iv: "12.4%", ivChg: "+2%" },
        atmPut: { val: 139.30, chg: "-18 (-11%)", iv: "13.1%", ivChg: "-1%" },
        straddle: { val: 284.50, chg: "-5 (-1%)", iv: "12.8%", ivChg: "+0.5%" },
        rv: "10.5%", ivr: "IVR 45", ivp: "IVP 60"
    },
    gridMonthly: {
        date: "29 JAN 2026",
        atmCall: { val: 310.50, chg: "+15 (5%)", iv: "14.2%", ivChg: "+1%" },
        atmPut: { val: 284.50, chg: "-12 (-4%)", iv: "14.8%", ivChg: "-0.3%" },
        straddle: { val: 595.00, chg: "+2 (0.4%)", iv: "14.5%", ivChg: "+0.8%" },
        rv: "11.0%", ivr: "IVR 52", ivp: "IVP 65"
    }
};
